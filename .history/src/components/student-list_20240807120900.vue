        ...
        <v-row>
            ...
            <v-col cols="10" class="table">
                ...
                <v-row class="wrap-buttons">
                    ...
                    <v-col cols="3">
                        <v-btn color="grey" class="ma-2 white--text"> Cadastrar Aluno
                        </v-btn>
                    </v-col>
                </v-row>
...
export default {
    ...
    methods: {
        ...
        async deleteStudent(student) {
            try {
                const response = await axios.delete(`http://localhost:3000/students?id=${student.id}`);
                this.students = response.data
            } catch (error) {
                console.log('Erro ao remover estudante!', error);
            }
        },
        async fetchStudents() {
            try {
